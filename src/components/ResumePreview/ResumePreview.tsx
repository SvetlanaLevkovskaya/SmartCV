...
export const ResumePreview = () => {
  ...
  const handleDownloadPDF = async () => {
    ...
    const fullName =
      personalInfo && typeof personalInfo.fullName === 'string'
          ...
          <div className="mb-6">
            ...
            {hasEmailOrPhone && (
              <p className="text-sm text-gray-600">
                ...
                {section.email && section.phoneNumber && <span className="px-2 text-gray-600">|</span>}
        ...
        return (
          <div className="mb-6">
            ...
            <div>
              ...
              {hasLocation && (
                <p className="text-sm text-gray-700">{[section.city, section.country].filter(Boolean).join(', ')}</p>
              )}

              {hasYears && (
                <p className="text-sm">{[section.startYear, section.endYear].filter(Boolean).join(' – ')}</p>
              )}
            </div>
          </div>
        );
  ...
  return (
    <section className="w-full p-6 pr-6 xl:pr-20 2xl:pr-40 min-h-screen">
      ...
      {sections.length > 0 && (
        <div id="resume-preview" className="border max-w-3xl mx-auto bg-white">
